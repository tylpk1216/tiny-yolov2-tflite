...
def non_maximal_suppression(results):
    if len(results) == 0:
        return []
    ...
    for row in range(GRID_HEIGHT):
        for column in range(GRID_WIDTH):
            for box in range(NUM_BOXES_PER_BLOCK):
                ...
                if confidence_in_class < THRESHOLD: continue
                ...
                print(msg)
                results.append([left, top, right, bottom, confidence_in_class, COCO_CLASSES[detected_class]])

    return non_maximal_suppression(results)
